...
async def start(client, message):
    ...
    if message.chat.type in [enums.ChatType.GROUP, enums.ChatType.SUPERGROUP]:
        buttons = [[
                    ...
                    InlineKeyboardButton('✪ Sᴜᴘᴘᴏʀᴛ Gʀᴏᴜᴘ', url="https://t.me/Luci_Movies"),
                    InlineKeyboardButton('⌬ Mᴏᴠɪᴇ Gʀᴏᴜᴘ', url='https://t.me/Luci_Movie_Group')
                ],[
                  InlineKeyboardButton('💌 SOURCE CODE 💌', url='https://t.me/+TS9prYhvTRc0Njk9')
                ],[
    ...
    if AUTH_CHANNEL and not await is_subscribed(client, message):
        ...
        btn = [
            ...
            ],[InlineKeyboardButton('MOVIE DATABASE🔥', url='https://t.me/+JA6WfwHzHGhhMTVl')],
            [InlineKeyboardButton('ADULT MOVIES 💦💦', url='https://t.me/+6vOySosoIqUzMzhl')],
            [InlineKeyboardButton('MAIN CHANNEL ', url='https://t.me/LUCI_MOVIES')],
        ]
                    ...
                    reply_markup=InlineKeyboardMarkup(
                        [
                            [
                                ...
                                InlineKeyboardButton('𝗕𝗢𝗧 𝗢𝗪𝗡𝗘𝗥', url="https://t.me/LuciG0D")
                            ],
                            [
    ...
    if not files_:
        ...
        try:
            ...
            title = '[ @LUCI_MOVIES ]   ' + ' '.join(filter(lambda x: not x.startswith('[') and not x.startswith('@'), file.file_name.split()))
    ...
    files = files_[0]
    title = '[ @LUCI_MOVIES ]   ' + ' '.join(filter(lambda x: not x.startswith('[') and not x.startswith('@'), files.file_name.split()))
    ...
    if CUSTOM_FILE_CAPTION:
        try:
            ...
            f_caption=f_caption
    if f_caption is None:
        f_caption = f"[ @LUCI_MOVIES ]  {' '.join(filter(lambda x: not x.startswith('[') and not x.startswith('@'), files.file_name.split()))}"
    if not await db.has_premium_access(message.from_user.id):
        if not await check_verification(client, message.from_user.id) and VERIFY == True:
            btn = [[
                InlineKeyboardButton("Verify", url=await get_token(client, message.from_user.id, f"https://telegram.me/{temp.U_NAME}?start="))
